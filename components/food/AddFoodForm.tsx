...
export default function AddFoodForm({ userType }: AddFoodFormProps) {
  ...
  const expiryOptions = useMemo(() => {
    ...
    todayTimes.forEach((timeStr) => {
      ...
      if (time > now) {
        options.push({ value: time.toISOString(), label: `Today at ${timeStr}` });
      }
    });
  ...
  const handleChange = (
    ...
    if (name === "allergens" && target instanceof HTMLSelectElement) {
      const selected = Array.from(target.selectedOptions, (o) => o.value as Allergen);
  ...
  const validateStep2 = (): Record<string, string> => {
    const newErrors: Record<string, string> = {};
    if (formData.isRaw) {
      newErrors.isRaw = "Only cooked, ready-to-eat food can be listed";
    }
    return newErrors;
  };
  ...
  const validateStep3 = (): Record<string, string> => {
    ...
    } else if (new Date(formData.expiresAt).getTime() <= Date.now()) {
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords: { latitude, longitude } }) => {
        try {
          ...
          setPickedLocation({ address: "Current Location", latitude, longitude });
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const combinedErrors = { ...validateStep1(), ...validateStep2(), ...validateStep3() };
    ...
    try {
      ...
      router.push(userType === "individual" ? "/protected/dashboard/individual" : "/protected/dashboard/restaurant");
    } catch (error) {
    ...
    <div className="min-h-screen bg-white dark:bg-gray-800 rounded-lg shadow-md shadow-gray-200 dark:shadow-gray-700 py-8">
      <div className="max-w-6xl mx-auto px-1 sm:px-2 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative">
          {/* Header */}
          <div className={`bg-linear-to-r ${accentGradient} rounded-3xl p-8 mb-8 text-white shadow-2xl`}>
            <div className="flex flex-col lg:flex-row gap-4 md:gap-0 items-center justify-between">
              <div className="flex items-start md:items-center gap-4">
                ...
                <div>
                  <h1 className="lg:text-3xl text-xl font-bold mb-2">
                    {userType === "restaurant" ? "Add Surplus Food" : "Share Home-Cooked Food"}
                  </h1>
              ...
              <Link href={userType === "individual" ? "/protected/dashboard/individual" : "/protected/dashboard/restaurant"}>
                <Button variant="outline" className="border-white hover:bg-white/20 w-full lg:w-auto">
                  Back to Dashboard
                </Button>
              </Link>
            </div>
          ...
          <div className="mb-8 w-full">
            ...
            <div className="grid grid-cols-3 text-center text-sm text-gray-600 dark:text-gray-300">
              ...
              <span>{userType === "individual" ? "Location & Safety" : "Safety Guidelines"}</span>
            ...
            <AnimatePresence mode="wait">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="dark:text-gray-100 text-gray-800 placeholder:text-gray-400 dark:placeholder:text-gray-500 w-full px-4 py-3 border-2 bg-gray-200 dark:bg-gray-800 dark:border-gray-300 border-gray-600 rounded-xl focus:outline-none shadow-sm focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 resize-none"
                      ...
                      <label className="flex items-start gap-4 cursor-pointer">
                        ...
                        <div className="flex-1">
                          <div className="font-semibold text-gray-900 dark:text-gray-50">
                            {userType === "individual" ? "Share as Donation" : "List as Donation"}
                          </div>
                    ...
                    {!formData.isDonation && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input
                          ...
                          onChange={handleChange}
                          placeholder={userType === "restaurant" ? "e.g., 299" : "e.g., 199"}
                          required
                        />
                        <Input
                      ...
                      <div className="space-y-4">
                            ...
                            <div className="flex w-full space-x-4">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                                  We&apos;ll use your device&apos;s GPS to set the pickup location
                                </div>
                              </div>
                            </div>
                                ...
                                {locationLoading ? (
                                  ...
                                  <div className="p-4 bg-white dark:bg-gray-900 rounded-xl">
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">
                                      Lat: {pickedLocation.latitude.toFixed(4)}, Lng:{" "}
                          ...
                          <div className="flex flex-col gap-4">
                            <div className="flex w-full space-x-4">
                              ...
                              <div>
                                ...
                                <div className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                                  Click the map or drag the pin to pick your exact location
                                </div>
                              </div>
                            </div>
                          ...
                          <div className="flex gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-yellow-700 dark:text-yellow-200 mt-1">
                                Choose a public meeting spot for pickup. Your exact address is only
                        ...
                        {!pickedLocation && (
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            No location picked — your profile&apos;s saved address will be used as
                            the pickup location instead.
                          </p>
                        )}
                      </div>
                    )}
                      ...
                      <div className="p-4 bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 rounded-xl">
                        <label className="flex items-start gap-4 cursor-pointer">
                          <input
                            type="checkbox"
                            name="isRaw"
                            checked={formData.isRaw}
                            onChange={handleChange}
                            className="mt-1 w-5 h-5 text-red-600 rounded"
                          />
                          <div>
                            <p className="font-medium text-red-900 dark:text-red-100">
                              This food contains raw ingredients
                            </p>
                            <p className="text-sm text-red-700 dark:text-red-200 mt-1">
                              Only cooked, ready-to-eat food is allowed on Annosetu
                            </p>
                          </div>
                        </label>
                      </div>
              ...
              {currentStep === 3 && (
                ...
                >
                  ...
                  <div className="space-y-6">
                    <div className="p-6 rounded-xl bg-linear-to-br from-orange-50 dark:from-orange-900 to-amber-50 dark:to-amber-900 border border-orange-200 dark:border-orange-700">
                      ...
                      />
                      {errors.expiresAt && (
                        <p className="text-sm text-red-600 mt-2">{errors.expiresAt}</p>
                      )}
 
                      {formData.expiresAt && (
                        <div className="mt-4 p-4 bg-white dark:bg-gray-800 rounded-xl">
                          <div className="text-center">
                            <p className="text-sm text-gray-600 dark:text-gray-300">Available for</p>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                              Auto-removes at {new Date(formData.expiresAt).toLocaleTimeString()}
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center hover:border-primary-400 transition-colors">
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-4">
                        Upload up to {MAX_IMAGES} images • Max 5MB each • PNG, JPG, GIF
                      </p>
                    </div>
          ...
          <motion.div
            ...
            className="mt-8 bg-linear-to-r from-red-50 dark:from-red-900 to-orange-50 dark:to-orange-900 border border-red-200 dark:border-red-700 rounded-2xl p-4"
          >
            <div className="flex gap-4">
              <FaShieldAlt className="w-8 h-8 text-red-500 dark:text-red-400 flex-shrink-0" />
              <div>
                <h3 className="font-semibold text-red-900 dark:text-red-50 mb-2">Food Safety First</h3>
                ...
                <p className="text-xs text-red-600 dark:text-red-300 mt-3">
                  Violation of safety guidelines may result in account suspension
                </p>
              </div>
            </div>
          </motion.div>
