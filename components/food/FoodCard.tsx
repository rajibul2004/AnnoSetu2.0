...
import { isFoodExpired, isFoodReserved, type PublicFoodDTO } from "@/types/food";
...
export default function FoodCard({ food, onReserve, isAuthenticated }: FoodCardProps) {
  const expired = isFoodExpired(food);
  const reserved = isFoodReserved(food);
  const canReserve = isAuthenticated && !expired && !reserved;
  const timeRemaining = formatTimeRemaining(food.expiresAt);
 
  const minutesRemaining = Math.floor((new Date(food.expiresAt).getTime() - Date.now()) / 60000);
  const getTimeColor = () => {
    if (minutesRemaining < 30) return "bg-red-100 text-red-800 border-red-200";
    if (minutesRemaining < 120) return "bg-orange-100 text-orange-800 border-orange-200";
    return "bg-green-100 text-green-800 border-green-200";
  };
 
  const getSupplierBadge = () => {
    if (food.supplierType === "restaurant") {
      return { bg: "bg-blue-100", text: "text-blue-800", icon: FaStore, label: "Restaurant" };
    }
    if (food.isHomeCooked) {
      return { bg: "bg-pink-100", text: "text-pink-800", icon: FaUser, label: "Home Cook" };
    }
    return {
      bg: "bg-purple-100",
  ...
  const availablePct =
    food.quantity > 0 ? Math.min(100, (food.availableQty / food.quantity) * 100) : 0;
          ...
          <div className="flex items-start gap-2 text-xs text-gray-500 dark:text-gray-400 mb-4">
            ...
            {food.distance !== null && (
              <>
                ...
                <span className="font-medium whitespace-nowrap">{food.distance.toFixed(1)} km</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="p-4 flex flex-col">
        <div className="mb-4">
          ...
          <div className="w-full h-1.5 bg-gray-100 dark:bg-gray-600 rounded-full overflow-hidden">
            <div
              className="h-full bg-linear-to-r from-green-500 to-amber-500 rounded-full transition-all duration-300"
        ...
        {expired ? (
          ...
          <button
            disabled
            className="w-full py-3 px-4 bg-yellow-50 text-yellow-700 rounded-xl font-medium cursor-not-allowed border border-yellow-200 flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="w-full cursor-pointer py-3 px-4 bg-linear-to-r from-green-500 to-amber-500 hover:from-green-600 hover:to-amber-600 text-white rounded-xl font-medium transition-all duration-200 hover:shadow-lg transform hover:scale-[1.02] flex items-center justify-center gap-2 group/btn"
          >
          ...
          <Link
            href="/login"
            className="w-full py-3 px-4 bg-white border-2 border-gray-200 hover:border-primary-500 text-gray-700 hover:text-primary-600 rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-2 group/btn"
          >
